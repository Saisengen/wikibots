...
class Program
{
    ...
    static void Main()
    {
        ...
        string cont, query = "https://ru.wikipedia.org/w/api.php?action=query&format=xml&list=unreviewedpages&urlimit=max&urnamespace=", apiout, result = "";
        foreach (var ns in nsnames.Keys)
        {
            ...
            while (cont != null)
            {
                apiout = (cont == "" ? site.GetStringAsync(query + ns).Result : site.GetStringAsync(query + ns + "&urcontinue=" + Uri.EscapeDataString(cont)).Result);
                using (var r = new XmlTextReader(new StringReader(apiout)))
                {
                    ...
                    while (r.Read())
                        if (r.Name == "p")
                            result += "#[[:" + r.GetAttribute("title") + "]]\n";
                }
            }
        }
        Save(site, "u:MBH/unreviewed pages", result, "");
    }
}
