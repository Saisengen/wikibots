...
class Program
{
    ...
    static void add_script(string scriptname)
    {
        ...
        scriptname = scriptname.Replace("_"," ").Replace("u:","user:").Replace("у:","user:").Replace("участник:","user:").Replace("участница:","user:").Replace("вп:","project:")
    ...
    static HashSet<string> scriptpages = new HashSet<string>();
    ...
    static Regex is_rgx = new Regex(@"importscript *\( *['""]([^h/][^""']*) *['""]", RegexOptions.IgnoreCase), is2_rgx = new Regex(@"importscript *\( *['""]/wiki/([^""']*) *['""]", RegexOptions.IgnoreCase),
        is_foreign_rgx = new Regex(@"importscript *\( *['""]([^h][^""']*)['""], *['""]([^""']*) *['""]", RegexOptions.IgnoreCase),
        is_ext_rgx = new Regex(@"importscript *\( *['""](https?:|)//([^.]*)\.([^.]*)\.org/wiki/([^""']*\.js)", RegexOptions.IgnoreCase),
        loader_rgx = new Regex(@"\.(load|getscript|using) *\( *['""]/w/index\.php\?title=(.*?\.js)", RegexOptions.IgnoreCase),
        loader_foreign_rgx = new Regex(@"\.(load|getscript|using) *\( *['""](https?:|)//([^.]*)\.([^.]*)\.org/w/index\.php\?title=(.*?\.js)", RegexOptions.IgnoreCase),
        r1 = new Regex("importscript", RegexOptions.IgnoreCase), r2 = new Regex(@"\.(load|getscript|using)", RegexOptions.IgnoreCase), multiline_comment = new Regex(@"/\*.*?\*/", RegexOptions.Singleline);
    ...
    static void Main()
    {
        ...
        {
            username = invoking_page.IndexOf('/') == -1 ? invoking_page.Substring(invoking_page.IndexOf(':') + 1) : invoking_page.Substring(invoking_page.IndexOf(':') + 1, invoking_page.IndexOf('/') - 1 - invoking_page.IndexOf(':'));
        ...
        string result = "{{shortcut|ВП:СИС}}<center>Статистика собирается по незакомментированным (//, /**/) включениям importScript и разнообразным методам load/using/getscript на скриптовых страницах " +
            "участников рувики, а также их global.js-файлах на Мете. Активные участники - сделавшие хоть одно действие за последний месяц.\n{|class=standard\n!Скрипт!!Активных!!Неактивных!!Всего";
