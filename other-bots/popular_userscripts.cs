...
class Program
{
    ...
    static void Save(HttpClient site, string title, string text, string comment)
    {
        ...
        if (result.ToString().Contains("uccess"))
            Console.WriteLine(DateTime.Now.ToString() + " written " + lang + ":" + title);
        else
            Console.WriteLine(result);
    }
    static bool user_is_active()
    {
        if (users_activity[lang].ContainsKey(username))
            return users_activity[lang][username];
        else
        {
            DateTime edit_ts = new DateTime(), log_ts = new DateTime();
            using (var r = new XmlTextReader(new StringReader(site[lang].GetStringAsync("https://" + lang + ".wikipedia.org/w/api.php?action=query&format=xml&list=usercontribs&uclimit=1&ucprop=timestamp&ucuser=" + Uri.EscapeUriString(username)).Result)))
                while (r.Read())
                    if (r.Name == "item")
                    {
            ...
            using (var r = new XmlTextReader(new StringReader(site[lang].GetStringAsync("https://" + lang + ".wikipedia.org/w/api.php?action=query&format=xml&list=logevents&leprop=timestamp&lelimit=1&leuser=" + Uri.EscapeUriString(username)).Result)))
                while (r.Read())
                    if (r.Name == "item")
                    {
                        string raw_ts = r.GetAttribute("timestamp");
            ...
            {
                users_activity[lang].Add(username, false); return false;
            }
            else
            {
                users_activity[lang].Add(username, true); return true;
            }
    ...
    static void add_script(string scriptname)
    {
        ...
        if (user_is_active() && scripts[lang].ContainsKey(scriptname))
            scripts[lang][scriptname].active++;
        else if (user_is_active() && !scripts[lang].ContainsKey(scriptname))
            scripts[lang].Add(scriptname, new data() { active = 1, inactive = 0 });
        else if (!user_is_active() && scripts[lang].ContainsKey(scriptname))
            scripts[lang][scriptname].inactive++;
        else
            scripts[lang].Add(scriptname, new data() { active = 0, inactive = 1 });
    }
    static void process_site(string url)
    {
        string content = "";
        using (var r = new XmlTextReader(new StringReader(site[lang].GetStringAsync(url).Result)))
            while (r.Read())
    ...
    static string username, lang, invoking_page, debug_result = "<center>\n{|class=\"standard sortable\"\n!Страница вызова!!Скрипт";
    static void Main()
    {
        var result = new Dictionary<string, string>() { { "ru", "[[К:Википедия:Статистика и прогнозы]]{{shortcut|ВП:СИС}}<center>Статистика собирается по незакомментированным включениям importScript/" +
                ...
                "последний месяц. Показаны лишь скрипты, имеющие более одного включения. Статистику использования гаджетов см. [[Special:GadgetUsage|тут]]. Подробная разбивка скриптов по страницам - " +
                "[[/details|тут]]. Обновлено " + DateTime.Now.ToString("dd.MM.yyyy") + ". \n{|class=\"standard sortable\"\n!Скрипт!!Активных!!Неактивных!!Всего" }, { "uk", "<center>\n{|class=\"wikitable " +
        ...
        foreach (string lang in new string[] { /*"uk",*/ "ru" })
        {
            ...
            foreach (var invoking_page in invoking_pages[lang])
            {
                ...
                if (!script_users[lang].Contains(username))
                    script_users[lang].Add(username);
            }
