...
class Program
{
    ...
    static void Main()
    {
        ...
        foreach (var mover in globalusers)
            using (var rdr = new XmlTextReader(new StringReader(site.GetStringAsync("https://ru.wikipedia.org/w/api.php?action=query&format=xml&list=usercontribs&uclimit=max&ucend=" + (DateTime.Now.Year) + "-01-01T00:00:00.000Z&ucprop=comment&ucuser=" + Uri.EscapeDataString(mover)).Result)))
                while (rdr.Read())
        ...
        if (globalusers_needs_flag.Count > 0)
        {
            ...
            string header = "{{/Шапка}}<!--Пожалуйста, не удаляйте эту строку, пишите ниже!-->\n";
            string newmessage = "==Выдать апата глобальным правщикам==\nПеречисленные ниже участники занимаются переименованием файлов на Викискладе с заменой включений во всех разделах. В соответствии с [[ВП:ПАТ#ГЛОБ]] прошу выдать им апата, чтобы такие правки не распатрулировали страницы.";
            foreach (var mover in globalusers_needs_flag)
                newmessage += "\n* [[special:contribs/" + mover + "|" + mover + "]]";
            newmessage += "\n~~~~\n\n";
            if (zkatext.Contains(header))
                Save(site, "ВП:Запросы к администраторам"/*"u:MBH/Черновик"*/, zkatext.Replace(header, header + "\n" + newmessage), "новые переименовывающие для выдачи апата");
            else
                Save(site, "ВП:Запросы к администраторам"/*"u:MBH/Черновик"*/, newmessage + zkatext, "новые переименовывающие для выдачи апата");
        }
