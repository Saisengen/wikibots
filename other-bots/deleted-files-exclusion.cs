...
class Program
{
    static string[] creds; static HttpClient ru, commons; static DateTime dtn = DateTime.UtcNow; static string univ_query;
    static void Main()
    {
        creds = new StreamReader((Environment.OSVersion.ToString().Contains("Windows") ? @"..\..\..\..\" : "") + "p").ReadToEnd().Split('\n');
        ru = Site("ru.wikipedia", creds[0], creds[1]); commons = Site("commons.wikimedia", creds[0], creds[1]);
    ...
    static string e(string src) { return Uri.EscapeDataString(src); } 
    static HttpClient Site(string wiki, string login, string password)
    {
        ...
        var result = client.GetAsync("https://" + wiki + ".org/w/api.php?action=query&meta=tokens&type=login&format=xml").Result; var doc = new XmlDocument();
        doc.LoadXml(result.Content.ReadAsStringAsync().Result); var logintoken = doc.SelectSingleNode("//tokens/@logintoken").Value;
        result = client.PostAsync("https://" + wiki + ".org/w/api.php", new FormUrlEncodedContent(new Dictionary<string, string> { { "action", "login" }, { "lgname", login }, { "lgpassword", password },
            { "lgtoken", logintoken } })).Result; return client;
    }
    static void Save(HttpClient site, string title, string text, string comment)
    {
        ...
        var token = doc.SelectSingleNode("//tokens/@csrftoken").Value; var request = new MultipartFormDataContent { { new StringContent("edit"), "action" }, { new StringContent(title), "title" },
            { new StringContent(text), "text" }, { new StringContent(comment), "summary" }, { new StringContent(token), "token" } }; result = site.PostAsync("https://ru.wikipedia.org/w/api.php", request).Result;
        if (!result.ToString().Contains("uccess")) Console.WriteLine(result);
    }
    static void delete_transclusion(pair dp, bool isCommons)
    {
        string initial_text = ru.GetStringAsync("https://ru.wikipedia.org/wiki/" + e(dp.page) + "?action=raw").Result; string new_page_text = initial_text;
    ...
    static void find_and_delete_usages(Dictionary<string, logrecord> deletedfiles, bool iscommons)
    {
        ...
        {
            using (var r = new XmlTextReader(new StringReader(commons.GetStringAsync("https://commons.wikimedia.org/w/api.php?action=query&format=xml&prop=info&titles=" + e(df)).Result)))
                while (r.Read())
                    if (r.Name == "page" && r.GetAttribute("_idx")[0] != '-')
                        deletedfiles.Remove(r.GetAttribute("title"));
            using (var r = new XmlTextReader(new StringReader(ru.GetStringAsync("https://ru.wikipedia.org/w/api.php?action=query&format=xml&prop=fileusage&fuprop=title&fulimit=max&titles=" + e(df)).Result)))
            {
    ...
    static void run_ru()
    {
        ...
        foreach (var rf in replacedfiles.Keys.ToList())
            using (var r = new XmlTextReader(new StringReader(ru.GetStringAsync("https://ru.wikipedia.org/w/api.php?action=query&format=xml&prop=fileusage&fuprop=title&fulimit=max&titles=" + e(rf)).Result)))
            {
                ...
                while (r.Read())
                {
                    if (r.NodeType == XmlNodeType.Element && r.Name == "page") {
                        ...
                        file_exists = r.GetAttribute("_idx")[0] != '-';
                    }
                    if (r.Name == "fu" && !file_exists && Convert.ToInt16(r.GetAttribute("ns")) % 2 == 0) {
                        var page = r.GetAttribute("title");
                        string initial_text = ru.GetStringAsync("https://ru.wikipedia.org/wiki/" + e(page) + "?action=raw").Result;
                        string newname;
                        try { newname = inner_link_to_replacement_file.Match(replacedfiles[rf].comment).Groups[3].Value; }
                        catch { newname = commons_importer_link.Match(replacedfiles[rf].comment).Groups[1].Value; }
