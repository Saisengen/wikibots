...
class MyBot : Bot
{
    ...
    public void mr1(Site site, string[] set, string[,] pages)
    {
        ...
        Page logpage = new Page(site, "Участник:Dibot/Лог напоминаний");
        logpage.Load();
        ...
        for (int i = 0; i < 5000; i++)
        {
            if (!string.IsNullOrEmpty(pages[i, 0]))
            {
                ...
                if (Convert.ToInt32(pages[i, 2]) > term)           // если посл.правка была более term дней назад, обрабатываем
                {
                    ...
                    try
                    {
                        n.Save("[[User:Dibot/RemindBot|напоминание о завершении срока]]", true);
                        log = log + "\n{{User:Dibot/Log|" + n.title + "|" + dnow.ToString("yyyy-MM-dd") + "|}}";
                    }
                    catch
                    {
                        try
                        ...
                        {
                            log = log + "\n{{User:Dibot/Log|" + n.title + "|" + dnow.ToString("yyyy-MM-dd") + "|не удалось сохранить}}";
                        }
                    }
                    //}
                }
        ...
        Regex logs = new Regex(@"{{User:Dibot.*?}}", RegexOptions.Singleline);
        ...
        foreach (Match m in logs.Matches(log))
        {
            ...
            {
                log = log.Replace(m.ToString(), "{{User:Dibot/Log|" + pagelog + "|" + datelog + "|шаблон снят}}");
            }
        }
        logpage.text = log;
        // сохраняем лог
    ...
    public void mr2(Site site, string[] set, string[,] pages)
    {
        ...
        DateTime dnow = DateTime.UtcNow;
        Page logpage = new Page(site, "Участник:Dibot/Лог уведомлений");
        logpage.Load();
        string log = logpage.text;
        for (int i = 0; i < 5000; i++)
        {
                ...
                if (Convert.ToInt32(pages[i, 1]) > term)           // если создано более N дней назад, обрабатываем
                {
                    ...
                    if (fromKU.Contains(n) != true) // если не с ВП:КУ
                    {
                        if (log.IndexOf(n.title) == -1) // если нет в логах
                        {
                            ...
                            try
                            {
                                n.Save("[[User:Dibot/RemindBot|уведомление о завершении срока]]", true);
                                log = log + "\n{{User:Dibot/Log|" + n.title + "|" + dnow.ToString("yyyy-MM-dd") + "|}}";
                            }
                            catch
                            {
    ...
    public void mrec(Site site, string[] set, string[,] pages)
    {
        ...
        foreach (Page n in forgotten)
        {
            ...
            try // на случай ошибок веб-запроса
            {
                ...
                pagetext = pagetext.Replace("  ", " ");
                pagetext = pagetext.Replace("  ", " ");
                for (int q1 = 0; q1 < 100; q1++) // (pagetext.IndexOf("\n\n") != -1)
                {
                    pagetext = pagetext.Replace("\n\n", "\n");
                }
                ...
                string titleforhist = n.title.Replace(" ", "_");
                titleforhist = titleforhist.Replace("\"", "%22");
                titleforhist = titleforhist.Replace("&", "%26");
                titleforhist = titleforhist.Replace("!", "&#33");
                titleforhist = titleforhist.Replace("!", "&#33");
        ...
        if (numpages > 0)
        {
            ...
            foreach (Page n in pact)
            {
                ...
                {
                    n.Save("[[User:Dibot/RemindBot|выставлено на мини-рецензирование]]", true);
                }
                catch // пробуем еще раз
                {
                    try { n.Save("[[User:Dibot/RemindBot|выставлено на мини-рецензирование]]", true); } // если повторно не вышло
                    catch
                    {
