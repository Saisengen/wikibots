...
class Program
{
    ...
    static void writerow(KeyValuePair<string, Dictionary<string, int>> s, bool per_year)
    {
        string newrow = "\n|-\n|" + ++position_number + "||{{u|" + s.Key + "}}||" + s.Value["sum"] + "||" + s.Value["К удалению"] + "||" + s.Value["К восстановлению"] + "||" + s.Value["К переименованию"] + 
            ...
            "||" + s.Value["Кандидаты в устаревшие избранные статьи"] + "||" + s.Value["Кандидаты в хорошие статьи/К лишению статуса"] + "||" + s.Value["Добротные статьи/К лишению статуса"] + "||" +
            s.Value["Кандидаты в избранные списки и порталы/К лишению статуса"] + "||" + s.Value["Запросы к ботоводам"] + "||" + (s.Value["Форум/Архив/Технический"] + s.Value["Технические запросы"]) + "||" +
            s.Value["Инкубатор/Мини-рецензирование"];
        if (per_year)
    ...
    static void Main()
    {
        ...
        string common_resulttext = "{{Плавающая шапка таблицы}}{{shortcut|ВП:ИТОГИ}}{{clear}}<center>{{самые активные участники}}\nСтатистика по числу итогов, подведённых %type%. См. также %otherpage%.\n\n" +
            ...
            "|0}}!!{{abbr|🔒|ЗС(-)|0}}!!{{abbr|👮⇧|ЗС(А)П|0}}!!{{abbr|👮⇧⇧|ЗСПИ|0}}!!{{abbr|👮⇩|ЗСФ|0}}!!{{abbr|📄⇩|КОИ|0}}!!{{abbr|🚫📄|ИСЛ|0}}!!{{abbr|✔|ЗКП(АУ)|0}}!!{{abbr|🥇|КИС|0}}!!{{abbr|🥈|" +
            "КХС|0}}!!{{abbr|🥉|КДС|0}}!!{{abbr|📜|КИСП|0}}!!{{abbr|🥇⇩|КИСЛ|0}}!!{{abbr|🥈⇩|КХСЛ|0}}!!{{abbr|🥉⇩|КДСЛ|0}}!!{{abbr|📜⇩|КИСПЛ|0}}!!{{abbr|🤖|РДБ|0}}!!{{abbr|🛠|ФТ+ТЗ|0}}!!{{abbr|👶|ИНК-МР|0}}";
