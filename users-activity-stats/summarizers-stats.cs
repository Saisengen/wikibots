...
class Program
{
    ...
    { { "month", new Dictionary<string, Dictionary<string, int>>() }, { "year", new Dictionary<string, Dictionary<string, int>>() } };
    ...
    static void writerow(KeyValuePair<string, Dictionary<string, int>> s, bool per_year)
    {
        string newrow = "\n|-\n|" + ++position_number + "||{{u|" + s.Key + "}}||" + cell(s.Value["sum"]) + "||" + cell(s.Value["К удалению"]) + "||" + cell(s.Value["К восстановлению"]) + "||" + cell(
            s.Value["К переименованию"]) + "||" + cell(s.Value["Запросы на переименование учётных записей"]) + "||" + cell(s.Value["К объединению"] + s.Value["К разделению"]) + "||" + cell(s.Value[
                ...
                "административных действий"]) + "||" + cell(s.Value["Установка защиты"] + s.Value["Снятие защиты"]) + "||" + cell(s.Value["Заявки на статус автопатрулируемого"] + s.Value["Заявки на статус " +
                "патрулирующего"]) + "||" + cell(s.Value["Заявки на статус подводящего итоги"]) + "||" + cell(s.Value["Заявки на снятие флагов"]) + "||" + cell(s.Value["К оценке источников"]) + "||" + cell(
                    s.Value["Изменение спам-листа"]) + "||" + cell(s.Value["Запросы к патрулирующим от автоподтверждённых участников"] + s.Value["Запросы к патрулирующим"]) + "||" + cell(s.Value["Кандидаты " +
                    "в избранные статьи"] + s.Value["Кандидаты в устаревшие избранные статьи"]) + "||" + cell(s.Value["Кандидаты в хорошие статьи"] + s.Value["Кандидаты в хорошие статьи/К лишению статуса"]) +
                    "||" + cell(s.Value["Кандидаты в добротные статьи"] + s.Value["Добротные статьи/К лишению статуса"]) + "||" + cell(s.Value["Кандидаты в избранные списки и порталы"] + s.Value["Кандидаты " +
                    "в избранные списки и порталы/К лишению статуса"]) + "||" + cell(s.Value["Запросы к ботоводам"]) + "||" + cell(s.Value["Форум/Архив/Технический"] + s.Value["Технические запросы"]) +
                    "||" + cell(s.Value["Форум/Архив/Авторское право"]) + "||" + cell(s.Value["Инкубатор/Мини-рецензирование"]);
        if (per_year)
    ...
    static void initialize(string type, string pagetype)
    {
        if (!stats[type].ContainsKey(user))
            stats[type].Add(user, new Dictionary<string, int>() { { "К удалению", 0 },{ "К улучшению", 0 },{ "К разделению", 0 },{ "К объединению", 0 },{ "К переименованию", 0 },{ "К восстановлению", 0 },
                ...
                { "Кандидаты в хорошие статьи/К лишению статуса", 0 },{ "Кандидаты в избранные списки и порталы/К лишению статуса", 0 }, { "Запросы на переименование учётных записей", 0},{ "Форум/Архив/Авторское право", 0 } });
        stats[type][user]["sum"]++;
    ...
    static void Main()
    {
        ...
        var first_not_fully_summaried_year = new Dictionary<string, int>
        {
            ...
            { "Кандидаты в избранные списки и порталы/К лишению статуса", 0 }, {"Запросы на переименование учётных записей", 0},{ "Форум/Архив/Авторское право", 0 }
        };
        ...
        string common_resulttext = "{{Плавающая шапка таблицы}}{{shortcut|ВП:ИТОГИ}}{{clear}}<center>{{самые активные участники}}\nСтатистика по числу итогов, подведённых %type%. См. также %otherpage%.\n\n" +
            ...
            "будет засчитан. Первично отсортировано по сумме всех итогов, кроме итогов на КУЛ и ЗКП(АУ).\n{|class=\"standard sortable ts-stickytableheader\"\n!№!!Участник!!Σ!!{{vertical header|[[ВП:КУ|]]}}!!" +
            "{{vertical header|[[ВП:ВУС|]]}}!!{{vertical header|[[ВП:КПМ|]]}}!!{{vertical header|[[ВП:ПУЗ|]]}}!!{{vertical header|[[ВП:КОБ|]]+[[ВП:КРАЗД|РАЗД]]}}!!{{vertical header|[[ВП:ОБК|]]}}!!" +
            "{{vertical header|[[ВП:КУЛ|]]}}!!{{vertical header|[[ВП:ЗКА|]]}}!!{{vertical header|[[ВП:ОСП|]]+[[ВП:ОАД|]]}}!!{{vertical header|[[ВП:ЗС|]][[ВП:ЗС-|(-)]]}}!!{{vertical header|[[ВП:ЗСП|ЗС]]+" +
            "[[ВП:ЗСАП|(А)П]]}}!!{{vertical header|[[ВП:ЗСПИ|]]}}!!{{vertical header|[[ВП:ЗСФ|]]}}!!{{vertical header|[[ВП:КОИ|]]}}!!{{vertical header|[[ВП:ИСЛ|]]}}!!{{vertical header|[[ВП:ЗКП|]][[ВП:ЗКПАУ|(АУ)]]}}" +
            "!!{{vertical header|[[ВП:КИС|]][[ВП:КИСЛ|(Л)]]}}!!{{vertical header|[[ВП:КХС|]][[ВП:КЛСХС|(Л)]]}}!!{{vertical header|[[ВП:КДС|]][[ВП:КЛСДС|(Л)]]}}!!{{vertical header|[[ВП:КИСП|]][[ВП:КЛСИСП|(Л)]]}}" +
            "!!{{vertical header|[[ВП:РДБ|]]}}!!{{vertical header|[[ВП:ФТ|]]+[[ВП:ТЗ|]]}}!!{{vertical header|[[ВП:Ф-АП|АП]]}}!!{{vertical header|[[ПРО:ИНК-МР|]]}}";
