...
class Program
{
    ...
    {
        string newrow = "\n|-\n|" + ++position_number + "||{{u|" + s.Key + "}}||" + s.Value["sum"] + "||" + s.Value["К удалению"] + "||" + s.Value["К восстановлению"] + "||" + s.Value["К переименованию"] + 
            ...
            "||" + (s.Value["Оспаривание итогов"] + s.Value["Оспаривание административных действий"]) + "||" + (s.Value["Установка защиты"] + s.Value["Снятие защиты"]) + "||" + (s.Value["Заявки на статус " +
            "автопатрулируемого"] + s.Value["Заявки на статус патрулирующего"]) + "||" + s.Value["Заявки на статус подводящего итоги"] + "||" + s.Value["Заявки на снятие флагов"] + "||" + 
            (s.Value["К оценке источников"] + s.Value["Изменение спам-листа"]) + "||" + (s.Value["Запросы к патрулирующим от автоподтверждённых участников"] + s.Value["Запросы к патрулирующим"]) + "||" +
            s.Value["Кандидаты в избранные статьи"] + "||" + s.Value["Кандидаты в хорошие статьи"] + "||" + s.Value["Кандидаты в добротные статьи"] + "||" + s.Value["Кандидаты в избранные списки и порталы"] +
            "||" + s.Value["Запросы к ботоводам"] + "||" + (s.Value["Форум/Архив/Технический"] + s.Value["Технические запросы"]) + "||" + s.Value["Инкубатор/Мини-рецензирование"];
        if (per_year)
    ...
    static void initialize(string type, string pagetype)
    {
        if (!stats[type].ContainsKey(user))
            stats[type].Add(user, new Dictionary<string, int>() { { "К удалению", 0 },{ "К улучшению", 0 },{ "К разделению", 0 },{ "К объединению", 0 },{ "К переименованию", 0 },{ "К восстановлению", 0 },
                ...
                { "Форум/Архив/Технический", 0 },{ "Запросы к патрулирующим от автоподтверждённых участников", 0 } });
        stats[type][user]["sum"]++;
    ...
    static void Main()
    {
        ...
        var first_not_fully_summaried_year = new Dictionary<string, int>
        {
            ...
            { "К оценке источников", 0 },{ "Изменение спам-листа", 0 },{ "Запросы к патрулирующим", 0 },{ "Запросы к патрулирующим от автоподтверждённых участников", 0 },
            { "Запросы к ботоводам", 0 },{ "Заявки на снятие флагов", 0 },{ "Запросы к администраторам", 0 },{ "Инкубатор/Мини-рецензирование", 0 },
            { "Кандидаты в хорошие статьи", 0 },{ "Кандидаты в избранные статьи", 0 },{ "Кандидаты в добротные статьи", 0 },{ "Кандидаты в избранные списки и порталы", 0 },
            { "Заявки на статус патрулирующего", 0 },{ "Заявки на статус подводящего итоги", 0 },{ "Заявки на статус автопатрулируемого", 0 },
        };
                ...
                foreach (var pageinfo in response.query.allpages)
                {
                    ...
                    if (correctpage)
                    {
                        ...
                        foreach (Match summary in summaries)
                        {
                            ...
                            user = summary.Groups[3].ToString().Replace('_', ' ');
                            if (user.Contains("/"))
                                continue;
        ...
        string common_resulttext = "{{Плавающая шапка таблицы}}{{shortcut|ВП:ИТОГИ}}{{clear}}<center>{{самые активные участники}}\nСтатистика по числу итогов, подведённых %type%. См. также %otherpage%.\n\n" +
            ...
            "{{abbr|<big>🗑⇧</big>|ВУС|0}}!!{{abbr|📄<big>⇨</big>|КПМ|0}}!!{{abbr|<big>⇨⇦</big>|КОБ|0}}<br>{{abbr|<big>⇦⇨</big>|КРАЗД|0}}!!{{abbr|<big>𓊍</big>|ОБК|0}}!!{{abbr|📄⇧|КУЛ|0}}!!{{abbr|🔨|ЗКА|0}}!!" +
            "{{abbr|💬⏮|ОСП+ОАД|0}}!!{{abbr|🔒|ЗС(-)|0}}!!{{abbr|👮⇧|ЗС(А)П|0}}!!{{abbr|👮⇧⇧|ЗСПИ|0}}!!{{abbr|👮⇩|ЗСФ|0}}!!{{abbr|🚫📄|КОИ+ИСЛ|0}}!!{{abbr|✔|ЗКП(АУ)|0}}!!{{abbr|🥇|КИС|0}}!!{{abbr|🥈|" +
            "КХС|0}}!!{{abbr|🥉|КДС|0}}!!{{abbr|📜|КИСП|0}}!!{{abbr|🤖|РДБ|0}}!!{{abbr|🛠|ФТ+ТЗ|0}}!!{{abbr|👶|ИНК-МР|0}}";
